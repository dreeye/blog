	...
	*/
	...
	{	
		...
		if ($this->form_validation->run() == FALSE)
		{
			...
			//获取数据列表
			$this->_data[$type] = $this->metas_mod->list_metas($type,$limit,$offset);
			$this->_data['total'] = $this->metas_mod->list_metas($type);
            if($this->_data['total']->num_rows() > $limit)
            {
                $this->load->library('dpagination');
                $this->dpagination->currentPage($page);
                $this->dpagination->items($this->_data['total']->num_rows());
                $this->dpagination->limit($limit);
                $this->dpagination->adjacents(2);
                $this->dpagination->target(site_url('admin/metas/manage/'.$this->_data['type']));
